...
function Restuarant() {
  ...
  let getRestuarantDetails = async () => {
    let URL = 'https://guarded-depths-42872.herokuapp.com/api/get-restuarant-by-id/' + params.id;
    try {
  ...
  let getMenuList = async () => {
    let URL = 'https://guarded-depths-42872.herokuapp.com/api/get-menu-item?rid=' + params.id;
    try {
      let response = await axios.get(URL);
      let data = response.data;
      if (data.status === true) {
