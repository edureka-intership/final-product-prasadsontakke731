...
function Wallpaper(props) {
  ...
  let getLocationList = async (event) => {
    ...
    let URL = 'https://guarded-depths-42872.herokuapp.com/api/get-location-by-city?city=' + city;
    try {
  ...
  let getRestuarantDetails = async (event) => {
    ...
    let URL = `https://guarded-depths-42872.herokuapp.com/api/get-restuarant-by-location-id?lid=${selectLoc.location_id}&rest=${restuarant}`;
    try {
      let response = await axios.get(URL);
